...
class DIFFormer(nn.Module):
    ...
    '''
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=3, num_heads=8, kernel='simple',
                 alpha=0.5, dropout=0.5, use_bn=True, use_residual=False, use_weight=True, use_graph=True):
    ...
    def forward(self, x, edge_index, edge_attr,edge_weight=None):
        ...
        for i, conv in enumerate(self.convs):
            ...
            if self.residual:
                y = layer_[i]
                # print(f"x:{x.shape},y:{y.shape},alpha:{self.alpha}")
                a = self.alpha*x
                b = (1-self.alpha) * y
