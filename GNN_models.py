...
def relu_kernel_transformation(data, is_query, projection_matrix=None, numerical_stabilizer=0.001):
    ...
    if projection_matrix is None:
        ...
        ratio = 1.0 / torch.sqrt(
            torch.tensor(projection_matrix.shape[0]).float()
        )
...
class NodeFormerConv(nn.Module):
    ...
    def forward(self, z, edge_index, tau):
        ...
        if self.use_edge_loss:
            if self.use_gumbel and self.training:  # only using Gumbel noise for training
                z_next, weight = kernelized_gumbel_softmax(query,key,value,self.kernel_transformation,projection_matrix,edge_index,
                                                    self.nb_gumbel_sample, tau, self.use_edge_loss)
            else:
                z_next, weight = kernelized_softmax(query, key, value, self.kernel_transformation, projection_matrix, edge_index,
                                                    tau, self.use_edge_loss)
        else:
            if self.use_gumbel and self.training: # only using Gumbel noise for training
                z_next = kernelized_gumbel_softmax(query,key,value,self.kernel_transformation,projection_matrix,edge_index,
                                                    self.nb_gumbel_sample, tau, self.use_edge_loss)
            else:
                z_next = kernelized_softmax(query, key, value, self.kernel_transformation, projection_matrix, edge_index,
                                                    tau, self.use_edge_loss)

        # compute update by relational bias of input adjacency, requires O(E)
        for i in range(self.rb_order):
            z_next += add_conv_relational_bias(value, edge_index, self.b[i], self.rb_trans)
...
class NodeFormer(nn.Module):
    ...
    '''
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=3, num_heads=8, dropout=0.0,
    ...
    def forward(self, x, edge_index, edge_attr,tau=1.0):
        x = x.unsqueeze(1)
        ...
        for i, conv in enumerate(self.convs):
            if self.use_edge_loss:
                z, link_loss = conv(z, edge_index, tau)
                link_loss_.append(link_loss)
            else:
                z = conv(z, edge_index, tau)
            if self.use_residual:
                z = z.unsqueeze(-2)
                y = layer_[i]
                # z += layer_[i].unsqueeze(-2)
                z = z + y
            if self.use_bn:
                z = self.bns[i+1](z)
            if self.use_act:
...
#     def __init__(self, in_channels, hidden_channels, out_channels, num_layers=3, num_heads=8, dropout=0.0,
#                  kernel_transformation=softmax_kernel_transformation, nb_random_features=30, use_bn=True, use_gumbel=True,
#                  use_residual=True, use_act=True, use_jk=False, nb_gumbel_sample=10, rb_order=0, rb_trans='sigmoid', use_edge_loss=True):
#         super(NodeFormer, self).__init__()
...
#         self.last_conv = NodeFormerConv(hidden_channels, out_channels, num_heads=num_heads, kernel_transformation=kernel_transformation,
#                                         nb_random_features=nb_random_features, use_gumbel=use_gumbel, nb_gumbel_sample=nb_gumbel_sample,
#                                         rb_order=rb_order, rb_trans=rb_trans, use_edge_loss=use_edge_loss)
...
#     def forward(self, x, edge_index, edge_attr, tau=1.):
#         x = x.unsqueeze(1)
#         x = x.unsqueeze(0)  # [B, N, H, D], B=1 denotes number of graph
...
#         # Apply the last convolutional layer followed by batch normalization
#         if self.use_edge_loss:
#             x_out, link_loss = self.last_conv(x_out, edge_index, tau)
#             link_loss_.append(link_loss)
#         else:
#             x_out = self.last_conv(x_out, edge_index, tau)
#         x_out = x_out.squeeze()
#         x_out = self.last_bn(x_out)
...
class GCN(nn.Module):
    ...
    def forward(self, x, edge_index):
        # x is the feature matrix of size [num_nodes, num_features]
        # edge_index is the adjacency matrix of size [2, num_edges]
...
class MPNN(nn.Module):
    def __init__(self, num_node_features, num_edge_features, hidden_dim, num_classes):
        ...
        self.conv1 = MPNNConv(num_node_features, num_edge_features, hidden_dim)
        self.conv2 = MPNNConv(hidden_dim, num_edge_features, num_classes)

    def forward(self, x, edge_index, edge_attr):
        # x is the node feature matrix of size [num_nodes, num_node_features]
        # edge_index is the adjacency matrix of size [2, num_edges]
        # edge_attr is the edge feature matrix of size [num_edges, num_edge_features]
...
class MPNNConv(MessagePassing):
    def __init__(self, num_node_features, num_edge_features, hidden_dim):
        ...
        self.lin_node = nn.Linear(num_node_features, hidden_dim)
        self.lin_edge = nn.Linear(num_edge_features, hidden_dim)
    ...
    def message(self, x_j, edge_attr):
        # x_j is the feature matrix of the neighboring nodes of size [num_edges, hidden_dim]
        # edge_attr is the feature matrix of the edges of size [num_edges, hidden_dim]
        return x_j + edge_attr

    def update(self, aggr_out):
        # aggr_out is the result of aggregating the messages of the neighboring nodes
        return aggr_out
    
    

class GNDT(MessagePassing):
    def __init__(self, in_channels, out_channels):
        super(GNDT, self).__init__(aggr='max')
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.lin = nn.Linear(in_channels, out_channels)
        self.W = nn.Parameter(torch.Tensor(out_channels, out_channels))
...
class GNDTModel(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes):
        super(GNDTModel, self).__init__()
        self.conv1 = GNDT(in_channels, out_channels)
        self.conv2 = GNDT(out_channels, out_channels)
...
class GraphSage(nn.Module):
    def __init__(self):
        ...
        self.conv3 = SAGEConv(in_channels=hidden_channels, out_channels=out_channels)
...
class GraphUNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.conv6 = GCNConv(hidden_dim * 2, hidden_dim)
        ...
        self.conv11 = GCNConv(hidden_dim * 2, output_dim)
    ...
    def forward(self, x, edge_index, batch):
        ...
        # Decoder
        x = F.relu(self.conv6(torch.cat([x5, x4], dim=1), edge_index))
        ...
        # Output
        x = self.conv11(torch.cat([x, x1], dim=1), edge_index)
